(ns clojurebot.bultitude.core
  (:require
   [clojure.java.io :as io]
   [clojure.string :as string]
   [clojurebot.dynapath.util :as dp])
  (:import
...
(defn path-for
  "Transform a namespace into a file path relative to classpath root, using the
  given extension (.clj default)."
  ([namespace]
...
(defn doc-from-ns-form
  "Extract the docstring from a given ns form without evaluating the form. The
  docstring returned should be the return value of (:doc (meta
  namespace-symbol)) if the ns-form were to be evaluated."
  [ns-form]
