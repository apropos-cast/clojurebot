(ns clojurebot.clojail.core
  (:use
   [clojure.stacktrace :only [root-cause]]
   [clojure.walk :only [walk postwalk-replace]]
   [clojurebot.clojail.jvm :only [permissions domain context
                                  jvm-sandbox]])
  (:import
...
(defn flatten-all
  ...
  Like clojure.core/flatten, but also works with maps and collections containing
  nested maps."
  [form]
  (remove coll? (tree-seq coll? seq form)))
...
(defn thunk-timeout
  ...
  ([thunk ms]
   ;; Default to milliseconds, because that's pretty common.
   (thunk-timeout thunk ms :ms nil))
  ([thunk time unit]
...
(defn safe-resolve
  ...
  (try (if-let [resolved (ns-resolve nspace s)]
         resolved
         s)
       ;; Catching both of these exceptions appears to be necessary, because the
       ;; exception that is thrown appears to be different depending on which
       ;; version of Clojure you use.
       (catch RuntimeException _ s)
       (catch ClassNotFoundException _ s)))

(defn- macroexpand-most
  "Macroexpand most, but not all. Leave non-collections and quoted things
  alone."
  [form]
  (if (or
       (not (coll? form))
...
(defn- make-dot
  ...
  `(defmacro ~'dot [object# method# & args#]
     `(let [~'obj# ~object#
            ~'obj-class# (class ~'obj#)]
        (if-let [~'bad# (some (partial clojurebot.clojail.core/unsafe? ~~tester-sym)
                              [~'obj-class# ~'obj# (.getPackage ~'obj-class#)])]
          (clojurebot.clojail.core/security-exception ~'bad#)
          (. ~object# ~method# ~@args#)))))
...
(defn- evaluator
  [code tester-sym tester-str context nspace transform bindings]
  (fn []
    (binding [*ns* nspace
              *read-eval* false]
      (let [bindings (or bindings {})
        ...
        (with-bindings bindings
          (jvm-sandbox #(transform (eval code)) context))))))

(defn set-security-manager
  "Sets the system security manager to whatever you pass. Passing nil is the
  equivalent of turning it off entirely (which is usually how the JVM starts
  up)."
  [s] (System/setSecurityManager s))

(defn sandbox*
  "This function creates a sandbox function that takes a tester. A tester is a
  set of objects that you don't want to be allowed in code. It is a blacklist.

  Optional arguments are as follows:

  :timeout, default is 10000 MS or 10 seconds. If the expression evaluated in
  the sandbox takes longer than the timeout, an error will be thrown and the
  thread running the code will be stopped.

  :namespace, the namespace of the sandbox. The default is (gensym \"sandbox\").

  :context, the context for the JVM sandbox to run in. Only relevant if :jvm? is
  true. It has a sane default, so you shouldn't need to worry about this.

  :jvm?, if set to true, the JVM sandbox will be employed. It defaults to true.

  :transform, a function to call on the result returned from the sandboxed code,
  before returning it, while still within the timeout context.

  :init, some (quoted) code to run in the sandbox's namespace, but outside of
  the sandbox.

  :refer-clojure, true or false. If true (the default), automatically
  refer-clojure in the ns.  You might want to set this to false at some point if
  you're working with the namespace in the :init key.

  This function will return a new function that you should bind to
  something. You can call this function with code and it will be evaluated in
  the sandbox. The function also takes an optional second parameter which is a
  hashmap of vars to values that will be passed to with-bindings. Since Clojure
  1.3, only vars explicitly declared as dynamic are able to be rebound. As a
  result, only those vars will work here. If this doesn't work for you, read
  about the :init key."
        ...
        (let [tester-str (pr-str tester)
          ...
          (try
            (if-let [problem (check-form code tester nspace)]
              ...
              (thunk-timeout
               (evaluator code tester-sym tester-str context nspace transform
                          bindings)
               timeout
               :ms
               (ThreadGroup. "sandbox")))
...
(defn sandbox
  "Convenience wrapper function around sandbox* to create a sandbox function out
   of a tester.

  Takes the same arguments as sandbox* with the addition of the tester
  argument. Returns a sandbox function like sandbox* returns, the difference
  being that the tester is hardcoded and doesn't need to be passed to the
  created function."
  [tester & args]
...
(defn safe-read
  ...
  them. read-eval errors will be thrown as IllegalStateException; other
  exceptions will be thrown unchanged."
  ([]
