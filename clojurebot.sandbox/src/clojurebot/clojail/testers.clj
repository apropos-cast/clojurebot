(ns clojurebot.clojail.testers
  ...
  (:require
   [clojurebot.bultitude.core :as nses]
   [clojurebot.serializable.fn :as sfn]))

(deftype ClojailWrapper [object])

(defmethod print-method ClojailWrapper
  [p out]
  (.write out (str "#clojurebot.clojail.testers.ClojailWrapper["
  ...
  (let [packages (map wrap-object packages)]
    (sfn/fn [obj]
      (let [obj (if (= Class (type obj))
                  ...
                  obj)]
        (when obj
          (when-let [result (first (filter #(let [pack (.object %)]
...
(defn blanket
  "Takes a tester and some namespace prefixes as strings. Looks up the prefixes
  with bultitude, getting a list of all namespaces on the classpath matching
  those prefixes."
  [& prefixes]
  (blacklist-nses (mapcat (partial nses/namespaces-on-classpath :prefix)
                          prefixes)))

(def ^{:doc "A tester that attempts to be secure, and allows def."}
  secure-tester-without-def
  [(blacklist-objects [clojure.lang.Compiler clojure.lang.Ref
                       clojure.lang.Reflector clojure.lang.Namespace
   ...
   (blacklist-symbols
    '#{alter-var-root intern eval catch load-string load-reader
       addMethod ns-resolve resolve find-var *read-eval*
       ns-publics ns-unmap set! ns-map ns-interns the-ns
       future-call agent send send-off pmap pcalls pvals in-ns
       System/out System/in System/err with-redefs-fn
       Class/forName})
